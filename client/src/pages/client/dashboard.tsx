...
export default function ClientDashboard() {
  ...
  const [showComprehensiveModal, setShowComprehensiveModal] =
    useState<any>(null);
  ...
  const openComprehensiveModal = (item: any) => {
    setShowComprehensiveModal(item);
  ...
  const closeComprehensiveModal = () => {
    setShowComprehensiveModal(null);
    setSelectedVideo(null);
    setIsPlaying(false);
    setModalActiveTab("preview");
    if (modalVideoRef.current) {
  ...
  return (
    <div className="min-h-screen bg-gray-50 dark:bg-black">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
                    {filteredMedia.map((item: any, index: number) => (
                      <Card
                        ...
                        onClick={() => openComprehensiveModal(item)}
                      >
      ...
      <Dialog
        ...
        onOpenChange={closeComprehensiveModal}
      >
        <DialogContent className="max-w-6xl w-[95vw] max-h-[90vh] p-0 bg-white dark:bg-gray-900">
          <DialogHeader className="px-6 py-4 border-b">
            <DialogTitle>{showComprehensiveModal?.title}</DialogTitle>
            <DialogDescription>
              {showComprehensiveModal?.type === "image" ? "Image" : "Video"} •
              Uploaded{" "}
              {showComprehensiveModal &&
                new Date(showComprehensiveModal.createdAt).toLocaleDateString()}
            </DialogDescription>
          </DialogHeader>
            ...
            <div className="flex flex-col h-full">
              ...
              <div className="flex-shrink-0 bg-black">
                <div className="aspect-video relative">
                  {showComprehensiveModal.type === "image" ? (
                    <img
                      src={showComprehensiveModal.url}
                      alt={showComprehensiveModal.title}
                    ...
                    <video
                      ...
                      onPause={() => {
                        ...
                        if (
                          ...
                          setNoteTimestamp(
                            Math.round(
                              modalVideoRef.current.currentTime * 100,
                            ) / 100,
                          );
                        }
                      }}
                ...
                {showComprehensiveModal.type === "video" && (
                    ...
                    <div className="flex items-center justify-between mt-4">
                      ...
                      <div className="flex items-center space-x-4">
                        <Button
                          ...
                          onClick={() =>
                            handleDownload(
                              showComprehensiveModal.url,
                              showComprehensiveModal.filename,
                            )
                          }
                          className="text-white hover:bg-white/20"
                        >
              ...
              <div className="flex-1 flex flex-col overflow-hidden">
                {/* Tab Navigation */}
                <div className="flex border-b bg-gray-50 dark:bg-gray-800 px-6">
                  <button
                      ...
                      <div className="grid grid-cols-2 gap-6">
                        <div>
                          ...
                          <div className="space-y-2 text-sm">
                            <div>
                              <strong>Title:</strong>{" "}
                              {showComprehensiveModal.title}
                            </div>
                            <div>
                              <strong>Type:</strong>{" "}
                              {showComprehensiveModal.type}
                            </div>
                            <div>
                              <strong>File Size:</strong>{" "}
                              {showComprehensiveModal.fileSize
                                ? `${(showComprehensiveModal.fileSize / 1024 / 1024).toFixed(2)} MB`
                                : "Unknown"}
                            </div>
                            <div>
                              <strong>Created:</strong>{" "}
                              {new Date(
                                showComprehensiveModal.createdAt,
                              ).toLocaleString()}
                            </div>
                          </div>
                        </div>
                          ...
                          <div className="space-y-2 text-sm">
                            <div>
                              ...
                              <Badge variant="outline" className="ml-2">
                                {showComprehensiveModal.projectStage ||
                                  "Not specified"}
                              </Badge>
                            </div>
                            <div>
                              <strong>Portfolio:</strong>{" "}
                              {showComprehensiveModal.showInPortfolio
                                ? "Visible"
                                : "Hidden"}
                            </div>
                            <div>
                              <strong>Featured:</strong>{" "}
                              {showComprehensiveModal.isFeatured ? "Yes" : "No"}
                            </div>
                          </div>
                        </div>
                      </div>

                      {showComprehensiveModal.tags &&
                        showComprehensiveModal.tags.length > 0 && (
                          <div>
                            <h4 className="font-semibold mb-3">Tags</h4>
                            <div className="flex flex-wrap gap-2">
                              {showComprehensiveModal.tags.map(
                                (tag: string) => (
                          ...
                          <Button
                            onClick={() => {
                              if (!feedbackText.trim() || rating === 0) {
                                toast({
                                  title:
                                    "Please provide both feedback text and rating",
                                });
                                return;
                              }
                              feedbackMutation.mutate({
                                mediaId: showComprehensiveModal.id,
                                feedbackText: feedbackText.trim(),
                                rating,
                              });
                            }}
                      ...
                      <div className="space-y-6">
                        ...
                        <div className="border rounded-lg p-4">
                          ...
                          <Button
                            ...
                            disabled={timelineNoteMutation.isPending}
                            size="sm"
                          >
                            {timelineNoteMutation.isPending ? (
                              <>
                        ...
                        <div>
                          ...
                          <div className="space-y-3 max-h-64 overflow-y-auto">
                            {typedTimelineNotes
                              .filter(
                                (note: any) =>
                                  note.mediaId === showComprehensiveModal.id,
                              )
                              .sort(
                                (a: any, b: any) =>
                            ...
                            {typedTimelineNotes.filter(
                              ...
                              <p className="text-sm text-gray-500 text-center py-8">
                                No timeline notes yet. Add your first note
                                above.
                              </p>
                            )}
                          </div>
