...
if (-not $Force) {
    Write-Host "Production Deployment Warning" -ForegroundColor Yellow
...
Write-Host "Creating default admin user for production..." -ForegroundColor Yellow
try {
    docker-compose -f docker-compose.prod.yml exec -T postgres-prod psql -U postgres -d dt_visuals_prod -c "SELECT create_default_admin();"
    Write-Host "Default admin user created successfully!" -ForegroundColor Green
    Write-Host "IMPORTANT: Login with admin@dtvisuals.com / admin123 and change the password immediately!" -ForegroundColor Red
}
catch {
    Write-Host "Admin user creation step encountered issues (this might be normal if user already exists)" -ForegroundColor Yellow
}
...
Write-Host "   SECURITY: Change the default password immediately after first login!" -ForegroundColor Red
Write-Host ""
